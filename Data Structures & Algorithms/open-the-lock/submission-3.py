class Solution:
    def openLock(self, deadends: List[str], target: str) -> int:
        ...
        pop value out of queue, loop through the 4 idx of popped val - the 2 possible steps are up and down, so for each of these idx need to perform this move of going up and down
        ...
        Once both checks pass, then check whether our moved combination meets our target, if it does then return turns - otherwise
        add the value to the set to mark it as seen, and add it onto the queue to be moved again
        '''
